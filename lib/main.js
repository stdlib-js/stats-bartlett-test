...
function bartlett() {
	...
	if ( opts.groups ) {
		...
		if ( ngroups < 2 ) {
			throw new Error( format( '0th8W,GY', 'groups', levels ) );
		}
		for ( i = 0; i < ngroups; i++ ) {
	...
	for ( i = 0; i < ngroups; i++ ) {
		arg = args[ i ];
		if ( !isCollection( arg ) ) {
			throw new TypeError( format( '0th8X,GZ', arg ) );
		}
		if ( arg.length === 0 ) {
			throw new Error( format( '0th8Y,Ga', arg ) );
		}
		n[ i ] = arg.length - 1;
		nSum += n[ i ];
		ninvSum += 1.0 / n[ i ];
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '0th8V,Gb', 'alpha', alpha ) );
	}
